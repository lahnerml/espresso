...
void lbadapt_stream_edges (p8est_iter_edge_info_t * info, void * user_data) {
  ...
  // p8est_quadrant_t       * quad; needed for adaptive grid
...
void lbadapt_stream_faces (p8est_iter_face_info_t * info, void * user_data) {
  ...
  lbadapt_payload_t      * ghost_data = (lbadapt_payload_t *) user_data;
  lbadapt_payload_t      * src, * dest;
  // p8est_quadrant_t       * quad; necessary for adaptivity
...
void lbadapt_bounceback_edges (p8est_iter_edge_info_t * info, void * user_data) {
  ...
  // depending on which face we're on we have to stream accordingly
  which_edge = side[0]->edge;
  switch (which_edge) {
    case 0:
      indexSrc = 16;
  ...
  if (side[0]->is_hanging) {
    ...
    // do not bounce back in halo cells
    if (side[0]->is.full.is_ghost) {
      return;
    } else {
      src = (lbadapt_payload_t *) side[0]->is.full.quad->p.user_data;
  ...
  if (src->boundary) {
    lbadapt_calc_modes (src->lbfluid, modes);

    // calculate population shift (moving boundary)
    population_shift = 0;
    for (l = 0; l < 3; l++) {
      population_shift -= h * SQR(SQR(h)) * // h**5
                        * lbpar.rho[0] * 2 * lbmodel.c[i][l] * lbmodel.w[i]
                        * lb_boundaries[src->boundary-1].velocity[l] / lbmodel.c_sound_sq;
    }
