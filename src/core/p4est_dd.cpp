...
void dd_p4est_comm () {
  ...
  for (int n=0;n<n_nodes;++n)
    for (int i=0;i<recv_idx[n].size();++i)
      fprintf(h,"%i:%li 0x%016lx\n",n,dd.p4est_shell[recv_idx[n][i]].idx,recv_tag[n][i]);
  fclose(h);*/
...
Cell* dd_p4est_save_position_to_cell(double pos[3]) {
  ...
  for (i=0;i<num_local_cells;++i) {
    if ((dd.p4est_shell[i].boundary & 1)) {
      if (scale_pos_l[0] < dd.p4est_shell[i].coord[0]) continue;
    } else {
      if (scale_pos[0] < dd.p4est_shell[i].coord[0]) continue;
    }
    if ((dd.p4est_shell[i].boundary & 2)) {
      if (scale_pos_h[0] > dd.p4est_shell[i].coord[0]) continue;
    } else {
      if (scale_pos[0] > dd.p4est_shell[i].coord[0]) continue;
    }
    if ((dd.p4est_shell[i].boundary & 4)) {
      if (scale_pos_l[1] < dd.p4est_shell[i].coord[1]) continue;
    } else {
      if (scale_pos[1] < dd.p4est_shell[i].coord[1]) continue;
    }
    if ((dd.p4est_shell[i].boundary & 8)) {
      if (scale_pos_h[1] > dd.p4est_shell[i].coord[1]) continue;
    } else {
      if (scale_pos[1] > dd.p4est_shell[i].coord[1]) continue;
    }
    if ((dd.p4est_shell[i].boundary & 16)) {
      if (scale_pos_l[2] < dd.p4est_shell[i].coord[2]) continue;
    } else {
      if (scale_pos[2] < dd.p4est_shell[i].coord[2]) continue;
    }
    if ((dd.p4est_shell[i].boundary & 32)) {
      if (scale_pos_h[2] > dd.p4est_shell[i].coord[2]) continue;
    } else {
      if (scale_pos[2] > dd.p4est_shell[i].coord[2]) continue;
    }
    break;
...
Cell* dd_p4est_position_to_cell(double pos[3]) {
  ...
  for (i=0;i<num_local_cells;++i) {
    ...
    //if (scale_pos[1] != dd.p4est_shell[i].coord[1]) continue;
    //if (scale_pos[2] != dd.p4est_shell[i].coord[2]) continue;
    if (pidx == dd_p4est_cell_morton_idx(dd.p4est_shell[i].coord[0],dd.p4est_shell[i].coord[1],dd.p4est_shell[i].coord[2]))
      break;
  }
...
void dd_p4est_partition(p4est_t *p4est, p4est_mesh_t *mesh, p4est_connectivity_t *conn) {
  ...
  for (int i=0;i<dd.p4est->local_num_quadrants;++i) {
    p4est_quadrant_t *q = p4est_mesh_get_quadrant(dd.p4est,mesh,i);
    double xyz[3];
  ...
  CELL_TRACE(printf("%i : repartition %i cells: ", this_node, dd.p4est->local_num_quadrants));
  for (int i=0;i<n_nodes;++i) {
  ...
  if (sum < dd.p4est->global_num_quadrants) {
  ...
  p4est_partition_given(dd.p4est, num_quad_per_proc_global);
}
//--------------------------------------------------------------------------------------------------
...
void dd_p4est_write_vtk() {
  ...
  //p4est_vtk_write_file_scale (dd.p4est, NULL, P4EST_STRING "_dd", box_l[0]/(double)brick_size[0]);
  
  /*char fname[100];
  sprintf(fname,"cells_conn_%i.list",this_node);
  FILE* h = fopen(fname,"w");
  for (int i=0;i<num_cells;++i) {
    fprintf(h,"%i %i:%li (%i) %i %i [ ",i, dd.p4est_shell[i].rank, dd.p4est_shell[i].idx,
      dd.p4est_shell[i].p_cnt, dd.p4est_shell[i].shell, dd.p4est_shell[i].boundary);
    for (int n=0;n<26;++n) fprintf(h,"%i ",dd.p4est_shell[i].neighbor[n]);
    fprintf(h,"]\n");
  }
  fclose(h);*/
