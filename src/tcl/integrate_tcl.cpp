...
int tclcommand_invalidate_system(ClientData data, Tcl_Interp *interp, int argc,
                                 char **argv) {
  Tcl_AppendResult(
      interp, "invalidate_system is no longer supported ",
...
int tclcommand_integrate_print_usage(Tcl_Interp *interp) {
  ...
  Tcl_AppendResult(interp, "'integrate <INT n steps> "
                           "[reuse_forces|recalc_forces]' for integrating n "
                           "steps\n",
                   (char *)NULL);
  Tcl_AppendResult(interp, "'integrate set' for printing integrator status \n",
                   (char *)NULL);
  Tcl_AppendResult(interp,
                   "'integrate set nvt' for enabling NVT integration or \n",
                   (char *)NULL);
#ifdef NPT
  Tcl_AppendResult(interp, "'integrate set npt_isotropic <DOUBLE p_ext> "
                           "[<DOUBLE piston>] [<INT, INT, INT "
                           "system_geometry>] [-cubic_box]' for enabling "
                           "isotropic NPT integration \n",
                   (char *)NULL);
#endif
  return (TCL_ERROR);
}
...
int tclcommand_integrate_set_npt_isotropic(Tcl_Interp *interp, int argc,
                                           char **argv) {
  int xdir, ydir, zdir;
  ...
  if (!ARG_IS_D(3, nptiso.p_ext))
    return tclcommand_integrate_print_usage(interp);
  tclcallback_p_ext(interp, &nptiso.p_ext);
  if (argc > 4) {
    if (!ARG_IS_D(4, nptiso.piston))
      return tclcommand_integrate_print_usage(interp);
    tclcallback_npt_piston(interp, &nptiso.piston);
  } else if (nptiso.piston <= 0.0) {
    Tcl_AppendResult(
        interp,
        "You must set <piston> as well before you can use this integrator! \n",
        (char *)NULL);
    return tclcommand_integrate_print_usage(interp);
  }
...
/* Sanity Checks */
  ...
  if (nptiso.dimension == 0 || nptiso.non_const_dim == -1) {
    Tcl_AppendResult(interp, "You must enable at least one of the x y z "
                             "components as fluctuating dimension(s) for box "
                             "length motion!",
                     (char *)NULL);
    Tcl_AppendResult(
        interp,
        "Cannot proceed with npt_isotropic, reverting to nvt integration... \n",
        (char *)NULL);
    integ_switch = INTEG_METHOD_NVT;
    mpi_bcast_parameter(FIELD_INTEG_SWITCH);
    return (TCL_ERROR);
...
int tclcallback_time_step(Tcl_Interp *interp, void *_data) {
  ...
  if (data < 0.0) {
    ...
    Tcl_AppendResult(interp, "MD time step must be smaller than LB time step.",
                     (char *)NULL);
    return (TCL_ERROR);
  }
#endif
...
int tclcallback_smaller_time_step(Tcl_Interp *interp, void *_data) {
  double data = *(double *)_data;
  if (data < 0.0) {
    Tcl_AppendResult(interp, "smaller time step must be positive.",
                     (char *)NULL);
    return (TCL_ERROR);
  } else if (data > time_step) {
    Tcl_AppendResult(
        interp, "smaller time step must be *smaller* than original time step.",
        (char *)NULL);
    return (TCL_ERROR);
  } else if ((int)(time_step / data) - (time_step / data) != 0.) {
    Tcl_AppendResult(interp,
                     "time_step/smaller_time_step needs to be an integer. %f",
                     (char *)NULL);
    return (TCL_ERROR);
  }
