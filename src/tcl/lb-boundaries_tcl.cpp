...
int tclcommand_lbboundary(ClientData _data, Tcl_Interp *interp, int argc,
                          char **argv);
int tclcommand_lbboundary_wall(LB_Boundary *lbb, Tcl_Interp *interp, int argc,
                               char **argv);
int tclcommand_lbboundary_sphere(LB_Boundary *lbb, Tcl_Interp *interp, int argc,
                                 char **argv);
int tclcommand_lbboundary_cylinder(LB_Boundary *lbb, Tcl_Interp *interp,
                                   int argc, char **argv);
int tclcommand_lbboundary_pore(LB_Boundary *lbb, Tcl_Interp *interp, int argc,
                               char **argv);
int tclcommand_lbboundary_stomatocyte(LB_Boundary *lbb, Tcl_Interp *interp,
                                      int argc, char **argv);
int tclcommand_lbboundary_hollow_cone(LB_Boundary *lbb, Tcl_Interp *interp,
                                      int argc, char **argv);
int tclcommand_lbboundary_voxel(LB_Boundary *lbb, Tcl_Interp *interp, int argc,
                                char **argv);
...
int tclcommand_printLbBoundaryToResult(Tcl_Interp *interp, int i) {
  ...
  case LB_BOUNDARY_SPHEROCYLINDER:
    Tcl_PrintDouble(interp, lbb->c.cyl.pos[0], buffer);
    Tcl_AppendResult(interp, "spherocylinder center ", buffer, " ",
                     (char *)NULL);
    Tcl_PrintDouble(interp, lbb->c.cyl.pos[1], buffer);
    Tcl_AppendResult(interp, buffer, " ", (char *)NULL);
    Tcl_PrintDouble(interp, lbb->c.cyl.pos[2], buffer);
  ...
  default:
    ...
    Tcl_AppendResult(interp, "unknown lbboundary type ", buffer, ".",
                     (char *)NULL);
  }
...
int tclcommand_lbboundary_wall(LB_Boundary *lbb, Tcl_Interp *interp, int argc,
  ...
  lbb->c.wal.n[0] = lbb->c.wal.n[1] = lbb->c.wal.n[2] = 0;
    ...
    if (ARG_IS_S(0, "normal")) {
      if (argc < 4) {
        Tcl_AppendResult(interp,
                         "lbboundary wall normal <nx> <ny> <nz> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "lbboundary wall dist <d> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "lbboundary wall type <t> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 4) {
        Tcl_AppendResult(interp,
                         "lbboundary wall velocity <vx> <vy> <vz> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
...
/* No velocity rescaling is required */
  ...
  if (norm < 1e-10) {
    Tcl_AppendResult(
        interp,
        "usage: lbboundary wall normal <nx> <ny> <nz> dist <d> type <t>",
        (char *)NULL);
    return (TCL_ERROR);
  }
...
int tclcommand_lbboundary_sphere(LB_Boundary *lbb, Tcl_Interp *interp, int argc,
  ...
  /* invalid entries to start of */
  lbb->c.sph.pos[0] = lbb->c.sph.pos[1] = lbb->c.sph.pos[2] = 0;
  lbb->c.sph.rad = 0;
  ...
  while (argc > 0) {
    if (ARG_IS_S(0, "center")) {
      if (argc < 4) {
        Tcl_AppendResult(interp,
                         "lbboundary sphere center <x> <y> <z> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "lbboundary sphere radius <r> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "-1/1 or inside/outside is expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (ARG_IS_S(1, "inside"))
        ...
        lbb->c.sph.direction = 1;
      else if (Tcl_GetDouble(interp, argv[1], &(lbb->c.sph.direction)) ==
               TCL_ERROR)
        return (TCL_ERROR);
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "lbboundary sphere type <t> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
  ...
  if (lbb->c.sph.rad < 0.) {
    Tcl_AppendResult(interp, "usage: lbboundary sphere center <x> <y> <z> "
                             "radius <d> direction <direction> type <t>",
                     (char *)NULL);
    return (TCL_ERROR);
  }
...
int tclcommand_lbboundary_cylinder(LB_Boundary *lbb, Tcl_Interp *interp,
  ...
  /* invalid entries to start of */
  lbb->c.cyl.pos[0] = lbb->c.cyl.pos[1] = lbb->c.cyl.pos[2] = 0;

  lbb->c.cyl.axis[0] = lbb->c.cyl.axis[1] = lbb->c.cyl.axis[2] = 0;
  ...
  while (argc > 0) {
    if (ARG_IS_S(0, "center")) {
      if (argc < 4) {
        Tcl_AppendResult(interp,
                         "lbboundary cylinder center <x> <y> <z> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 4) {
        Tcl_AppendResult(interp,
                         "lbboundary cylinder axis <rx> <ry> <rz> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "lbboundary cylinder radius <rad> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "lbboundary cylinder length <len> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "lbboundary cylinder direction <dir> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (ARG_IS_S(1, "inside"))
        ...
        lbb->c.cyl.direction = 1;
      else if (Tcl_GetDouble(interp, argv[1], &(lbb->c.cyl.direction)) ==
               TCL_ERROR)
        return (TCL_ERROR);
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "lbboundary cylinder type <t> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 4) {
        Tcl_AppendResult(interp,
                         "lbboundary cylinder velocity <vx> <vy> <vz> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
...
/* No velocity rescaling is required */
  ...
  if (lbb->c.cyl.rad < 0. || axis_len < 1e-30 || lbb->c.cyl.direction == 0 ||
      lbb->c.cyl.length <= 0) {
    Tcl_AppendResult(interp, "usage: lbboundary cylinder center <x> <y> <z> "
                             "axis <rx> <ry> <rz> radius <rad> length <length> "
...
int tclcommand_lbboundary_spherocylinder(LB_Boundary *lbb, Tcl_Interp *interp,
                                         int argc, char **argv) {
  double axis_len;
  int i;
  ...
  /* invalid entries to start of */
  lbb->c.spherocyl.pos[0] = lbb->c.spherocyl.pos[1] = lbb->c.spherocyl.pos[2] =
      0;

  lbb->c.spherocyl.axis[0] = lbb->c.spherocyl.axis[1] =
  ...
  while (argc > 0) {
    if (ARG_IS_S(0, "center")) {
      if (argc < 4) {
        Tcl_AppendResult(
            interp, "lbboundary spherocylinder center <x> <y> <z> expected",
            (char *)NULL);
        return (TCL_ERROR);
      }

      if (Tcl_GetDouble(interp, argv[1], &(lbb->c.spherocyl.pos[0])) ==
              TCL_ERROR ||
          Tcl_GetDouble(interp, argv[2], &(lbb->c.spherocyl.pos[1])) ==
              TCL_ERROR ||
          Tcl_GetDouble(interp, argv[3], &(lbb->c.spherocyl.pos[2])) ==
              TCL_ERROR)
        return (TCL_ERROR);
      ...
      if (argc < 4) {
        Tcl_AppendResult(
            interp, "lbboundary spherocylinder axis <rx> <ry> <rz> expected",
            (char *)NULL);
        return (TCL_ERROR);
      }

      if (Tcl_GetDouble(interp, argv[1], &(lbb->c.spherocyl.axis[0])) ==
              TCL_ERROR ||
          Tcl_GetDouble(interp, argv[2], &(lbb->c.spherocyl.axis[1])) ==
              TCL_ERROR ||
          Tcl_GetDouble(interp, argv[3], &(lbb->c.spherocyl.axis[2])) ==
              TCL_ERROR)
        return (TCL_ERROR);
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp,
                         "lbboundary spherocylinder radius <rad> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp,
                         "lbboundary spherocylinder length <len> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }

      if (Tcl_GetDouble(interp, argv[1], &(lbb->c.spherocyl.length)) ==
          TCL_ERROR)
        return (TCL_ERROR);
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp,
                         "lbboundary spherocylinder direction <dir> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (ARG_IS_S(1, "inside"))
        ...
        lbb->c.spherocyl.direction = 1;
      else if (Tcl_GetDouble(interp, argv[1], &(lbb->c.spherocyl.direction)) ==
               TCL_ERROR)
        return (TCL_ERROR);
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "lbboundary spherocylinder type <t> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 4) {
        Tcl_AppendResult(
            interp,
            "lbboundary spherocylinder velocity <vx> <vy> <vz> expected",
            (char *)NULL);
        return (TCL_ERROR);
      }
...
/* No velocity rescaling is required */
  ...
  if (lbb->c.spherocyl.rad < 0. || axis_len < 1e-30 ||
    ...
    Tcl_AppendResult(interp, "usage: lbboundary spherocylinder center <x> <y> "
                             "<z> axis <rx> <ry> <rz> radius <rad> length "
                             "<length> direction <direction> type <t>",
                     (char *)NULL);
    return (TCL_ERROR);
  }
...
int tclcommand_lbboundary_rhomboid(LB_Boundary *lbb, Tcl_Interp *interp,
                                   int argc, char **argv) {
  ...
  lbb->c.rhomboid.pos[0] = lbb->c.rhomboid.pos[1] = lbb->c.rhomboid.pos[2] = 0;

  lbb->c.rhomboid.a[0] = lbb->c.rhomboid.a[1] = lbb->c.rhomboid.a[2] = 0;

  lbb->c.rhomboid.b[0] = lbb->c.rhomboid.b[1] = lbb->c.rhomboid.b[2] = 0;

  lbb->c.rhomboid.c[0] = lbb->c.rhomboid.c[1] = lbb->c.rhomboid.c[2] = 0;
  ...
  while (argc > 0) {
    if (ARG_IS_S(0, "a")) {
      if (argc < 4) {
        Tcl_AppendResult(interp,
                         "lbboundary rhomboid a <ax> <ay> <az> expected",
                         (char *)NULL);
        return TCL_ERROR;
      }

      if (Tcl_GetDouble(interp, argv[1], &(lbb->c.rhomboid.a[0])) ==
              TCL_ERROR ||
          Tcl_GetDouble(interp, argv[2], &(lbb->c.rhomboid.a[1])) ==
              TCL_ERROR ||
      ...
      if (argc < 4) {
        Tcl_AppendResult(interp,
                         "lbboundary rhomboid b <bx> <by> <bz> expected",
                         (char *)NULL);
        return TCL_ERROR;
      }

      if (Tcl_GetDouble(interp, argv[1], &(lbb->c.rhomboid.b[0])) ==
              TCL_ERROR ||
          Tcl_GetDouble(interp, argv[2], &(lbb->c.rhomboid.b[1])) ==
              TCL_ERROR ||
      ...
      if (argc < 4) {
        Tcl_AppendResult(interp,
                         "lbboundary rhomboid c <cx> <cy> <cz> expected",
                         (char *)NULL);
        return TCL_ERROR;
      }

      if (Tcl_GetDouble(interp, argv[1], &(lbb->c.rhomboid.c[0])) ==
              TCL_ERROR ||
          Tcl_GetDouble(interp, argv[2], &(lbb->c.rhomboid.c[1])) ==
              TCL_ERROR ||
      ...
      if (argc < 4) {
        Tcl_AppendResult(interp,
                         "lbboundary rhomboid corner <x> <y> <z> expected",
                         (char *)NULL);
        return TCL_ERROR;
      }

      if (Tcl_GetDouble(interp, argv[1], &(lbb->c.rhomboid.pos[0])) ==
              TCL_ERROR ||
          Tcl_GetDouble(interp, argv[2], &(lbb->c.rhomboid.pos[1])) ==
              TCL_ERROR ||
          Tcl_GetDouble(interp, argv[3], &(lbb->c.rhomboid.pos[2])) ==
              TCL_ERROR)
        return TCL_ERROR;
      ...
      if (argc < 4) {
        Tcl_AppendResult(interp,
                         "lbboundary rhomboid velocity <vx> <vy> <vz> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
...
/* No velocity rescaling is required */
      ...
      if (argc < 2) {
        Tcl_AppendResult(
            interp, "lbboundary rhomboid direction {inside|outside} expected",
            (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (ARG_IS_S(1, "inside"))
        ...
        lbb->c.rhomboid.direction = 1;
      else if (Tcl_GetDouble(interp, argv[1], &(lbb->c.rhomboid.direction)) ==
               TCL_ERROR)
        return TCL_ERROR;
      ...
      Tcl_AppendResult(interp, "Error: Unknown parameter ", argv[0],
                       " in lbboundary rhomboid", (char *)NULL);
      return TCL_ERROR;
    }
  }

  if ((lbb->c.rhomboid.a[0] == 0. && lbb->c.rhomboid.a[1] == 0. &&
       lbb->c.rhomboid.a[2] == 0.) ||
      (lbb->c.rhomboid.b[0] == 0. && lbb->c.rhomboid.b[1] == 0. &&
       lbb->c.rhomboid.b[2] == 0.) ||
      (lbb->c.rhomboid.c[0] == 0. && lbb->c.rhomboid.c[1] == 0. &&
       lbb->c.rhomboid.c[2] == 0.) ||
      lbb->c.rhomboid.direction == 0) {
    Tcl_AppendResult(interp, "usage: lbboundary rhomboid corner <x> <y> <z> a "
                             "<ax> <ay> <az> b <bx> <by> <bz> c <cx> <cy> <cz> "
                             "direction {inside|outside} type <t> [penetrable "
                             "<0|1>] [reflecting <1|2>]",
                     (char *)NULL);
    return TCL_ERROR;
  }

  // If the trihedron a, b, c is left handed, then inside and outside will be
  // exchanged since all normals will be reversed. This compensates  for that,
  // so that the user doesn't have to take care of the handedness.
  triple_product =
      lbb->c.rhomboid.a[0] * (lbb->c.rhomboid.b[1] * lbb->c.rhomboid.c[2] -
                              lbb->c.rhomboid.b[2] * lbb->c.rhomboid.c[1]) +
      lbb->c.rhomboid.a[1] * (lbb->c.rhomboid.b[2] * lbb->c.rhomboid.c[0] -
                              lbb->c.rhomboid.b[0] * lbb->c.rhomboid.c[2]) +
      lbb->c.rhomboid.a[2] * (lbb->c.rhomboid.b[0] * lbb->c.rhomboid.c[1] -
                              lbb->c.rhomboid.b[1] * lbb->c.rhomboid.c[0]);
...
int tclcommand_lbboundary_pore(LB_Boundary *lbb, Tcl_Interp *interp, int argc,
  ...
  /* invalid entries to start of */
  lbb->c.pore.pos[0] = lbb->c.pore.pos[1] = lbb->c.pore.pos[2] = 0;

  lbb->c.pore.axis[0] = lbb->c.pore.axis[1] = lbb->c.pore.axis[2] = 0;
  ...
  while (argc > 0) {
    if (ARG_IS_S(0, "center")) {
      if (argc < 4) {
        Tcl_AppendResult(interp, "lbboundary pore center <x> <y> <z> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 4) {
        Tcl_AppendResult(interp, "lbboundary pore axis <rx> <ry> <rz> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "lbboundary pore radius <rad> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "lbboundary pore outer_radius <rad> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }

      if (Tcl_GetDouble(interp, argv[1], &(lbb->c.pore.outer_rad_left)) ==
          TCL_ERROR)
        return (TCL_ERROR);
      ...
      if (argc < 1) {
        Tcl_AppendResult(
            interp,
            "lbboundary pore smoothing_radius <smoothing_radius> expected",
            (char *)NULL);
        return (TCL_ERROR);
      }

      if (Tcl_GetDouble(interp, argv[1], &(lbb->c.pore.smoothing_radius)) ==
          TCL_ERROR)
        return (TCL_ERROR);
      ...
      if (argc < 1) {
        Tcl_AppendResult(
            interp, "lbboundary pore radii <rad_left> <rad_right> expected",
            (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(
            interp,
            "lbboundary pore outer_radii <rad_left> <rad_right> expected",
            (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (Tcl_GetDouble(interp, argv[2], &(lbb->c.pore.outer_rad_right)) ==
          TCL_ERROR)
        return (TCL_ERROR);
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "lbboundary pore length <len/2> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
  ...
  if (lbb->c.pore.rad_left < 0. || lbb->c.pore.rad_right < 0. ||
      axis_len < 1e-30 || lbb->c.pore.length <= 0) {
    Tcl_AppendResult(interp, "usage: lbboundary pore center <x> <y> <z> axis "
                             "<rx> <ry> <rz> radius <rad> length <length/2>",
                     (char *)NULL);
    return (TCL_ERROR);
  }
...
int tclcommand_lbboundary_stomatocyte(LB_Boundary *lbb, Tcl_Interp *interp,
  ...
  while (argc > 0) {
    if (ARG_IS_S(0, "center")) {
      if (argc < 4) {
        Tcl_AppendResult(interp,
                         "lbboundary stomatocyte center <x> <y> <z> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 4) {
        Tcl_AppendResult(
            interp,
            "lbboundary stomatocyte orientation <ox> <oy> <oz> expected",
            (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 2) {
        Tcl_AppendResult(interp,
                         "lbboundary stomatocyte outer_radius <Ro> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 2) {
        Tcl_AppendResult(interp,
                         "lbboundary stomatocyte inner_radius <Ri> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 2) {
        Tcl_AppendResult(interp,
                         "lbboundary stomatocyte layer_width <w> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 2) {
        Tcl_AppendResult(interp, "lbboundary stomatocyte direction {-1|1} or "
                                 "{inside|outside} is expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
  ...
  if (lbb->c.stomatocyte.outer_radius < 0.0 ||
    ...
    Tcl_AppendResult(interp,
                     "stomatocyte radii and width have to be greater than zero",
                     (char *)NULL);
    return (TCL_ERROR);
  }
  ...
  if (lbb->c.stomatocyte.outer_radius < lbb->c.stomatocyte.inner_radius ||
    ...
    Tcl_AppendResult(
        interp,
        "stomatocyte requires layer_width < inner_radius < outer_radius",
        (char *)NULL);
    return (TCL_ERROR);
  }
...
int tclcommand_lbboundary_hollow_cone(LB_Boundary *lbb, Tcl_Interp *interp,
  ...
  while (argc > 0) {
    if (ARG_IS_S(0, "center")) {
      if (argc < 4) {
        Tcl_AppendResult(interp,
                         "lbboundary hollow_cone center <x> <y> <z> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 4) {
        Tcl_AppendResult(
            interp,
            "lbboundary hollow_cone orientation <ox> <oy> <oz> expected",
            (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 2) {
        Tcl_AppendResult(interp,
                         "lbboundary hollow_cone outer_radius <Ro> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 2) {
        Tcl_AppendResult(interp,
                         "lbboundary hollow_cone inner_radius <Ri> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 2) {
        Tcl_AppendResult(interp, "lbboundary hollow_cone width <w> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 2) {
        Tcl_AppendResult(
            interp, "lbboundary hollow_cone opening_angle <alpha> expected",
            (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 2) {
        Tcl_AppendResult(interp, "lbboundary hollow_cone direction {-1|1} or "
                                 "{inside|outside} is expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
  ...
  if (lbb->c.hollow_cone.outer_radius < 0.0 ||
      lbb->c.hollow_cone.inner_radius < 0.0 || lbb->c.hollow_cone.width < 0.0) {
    Tcl_AppendResult(interp,
                     "hollow_cone radii and width have to be greater than zero",
                     (char *)NULL);
    return (TCL_ERROR);
  }
  ...
  if (fabs(fmod(lbb->c.hollow_cone.outer_radius, 1.0)) < 1.0e-05 ||
    ...
    fprintf(stderr, "Warning: Using (almost) exact integer values for the "
                    "radii or width.\n");
    fprintf(stderr, "         can lead to numerical problems when the LB grid "
                    "points coincide\n");
    fprintf(
        stderr,
        "         with the lattice, for specific values of the position and\n");
    fprintf(stderr, "         orientation. Consider adding or subtracting a "
                    "small number\n");
    fprintf(
        stderr,
        "         to/from the specified sizes to overcome such problems.\n");
    fflush(stdout);
  }
...
int tclcommand_lbboundary_voxel(LB_Boundary *lbb, Tcl_Interp *interp, int argc,
  ...
  // if (argc < 5 ) {
  // Tcl_AppendResult(interp, "lbboundaries voxel file <filename> type <type>
  // expected", (char *) NULL);
  // return (TCL_ERROR);
  //}
  ...
  while (argc > 0) {
    ...
    // if(ARG_IS_S(0, "pos")) {
    // if(argc < 4) {
    // Tcl_AppendResult(interp, "lbboundary voxel pos <x> <y> <z> expected",
    // (char *) NULL);
    // return (TCL_ERROR);
    //}
    ...
    if (ARG_IS_S(0, "file")) {
      if (argc < 1) {
        Tcl_AppendResult(interp, "lbboundary voxel file <filename> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      fp = fopen(argv[1], "r");
      if (!fp) {
        Tcl_AppendResult(interp, "voxel input file could not be opened",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      fclose(fp);
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "lbboundary voxel type <t> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
...
int tclcommand_lbboundary_box(LB_Boundary *lbb, Tcl_Interp *interp, int argc,
                              char **argv) {
...
int tclcommand_lbboundary(ClientData data, Tcl_Interp *interp, int argc,
                          char **argv) {
#if defined(LB_BOUNDARIES) || defined(LB_BOUNDARIES_GPU)
  int status = TCL_ERROR, c_num;

  if (lattice_switch == LATTICE_OFF) {
    fprintf(stderr, "WARNING: Specifying boundaries before using lbfluid "
                    "assumes a CPU implementation of the LB.\n");
    fprintf(stderr, "WARNING: This will lead to unexpected behavior if a GPU "
                    "LB fluid is later used since the boundaries wont "
                    "exist.\n");
  }
  ...
  if (ARG_IS_S(1, "wall")) {
    status = tclcommand_lbboundary_wall(generate_lbboundary(), interp, argc - 2,
                                        argv + 2);
    if (lattice_switch & LATTICE_LB_GPU) {
    ...
    status = tclcommand_lbboundary_sphere(generate_lbboundary(), interp,
                                          argc - 2, argv + 2);
    if (lattice_switch & LATTICE_LB_GPU) {
      mpi_bcast_lbboundary(-3);
    } else
    ...
    status = tclcommand_lbboundary_cylinder(generate_lbboundary(), interp,
                                            argc - 2, argv + 2);
    if (lattice_switch & LATTICE_LB_GPU) {
      mpi_bcast_lbboundary(-3);
    } else
    ...
    status = tclcommand_lbboundary_spherocylinder(generate_lbboundary(), interp,
                                                  argc - 2, argv + 2);
    if (lattice_switch & LATTICE_LB_GPU) {
      mpi_bcast_lbboundary(-3);
    } else
    ...
    status = tclcommand_lbboundary_rhomboid(generate_lbboundary(), interp,
                                            argc - 2, argv + 2);
    if (lattice_switch & LATTICE_LB_GPU) {
      mpi_bcast_lbboundary(-3);
    } else
    ...
    status = tclcommand_lbboundary_pore(generate_lbboundary(), interp, argc - 2,
                                        argv + 2);
    if (lattice_switch & LATTICE_LB_GPU) {
      mpi_bcast_lbboundary(-3);
    } else
    ...
    status = tclcommand_lbboundary_stomatocyte(generate_lbboundary(), interp,
                                               argc - 2, argv + 2);
    if (lattice_switch & LATTICE_LB_GPU) {
      mpi_bcast_lbboundary(-3);
    } else
    ...
    status = tclcommand_lbboundary_hollow_cone(generate_lbboundary(), interp,
                                               argc - 2, argv + 2);
    if (lattice_switch & LATTICE_LB_GPU) {
      mpi_bcast_lbboundary(-3);
    } else
    ...
    status = tclcommand_lbboundary_voxel(generate_lbboundary(), interp,
                                         argc - 2, argv + 2);
    if (lattice_switch & LATTICE_LB_GPU) {
      mpi_bcast_lbboundary(-3);
    } else
    ...
    if (c_num < 0 || c_num >= n_lb_boundaries) {
      Tcl_AppendResult(
          interp,
          "Error in lbboundary force: The selected boundary does not exist",
          (char *)NULL);
      return (TCL_ERROR);
    }
    ...
    if (argc < 3) {
      ...
      if (lattice_switch & LATTICE_LB_GPU) {
        Tcl_AppendResult(interp, "Cannot delete individual lb boundaries",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (Tcl_GetInt(interp, argv[2], &(c_num)) == TCL_ERROR)
        return (TCL_ERROR);
      if (c_num < 0 || c_num >= n_lb_boundaries) {
        Tcl_AppendResult(interp, "Can not delete non existing lbboundary",
                         (char *)NULL);
        return (TCL_ERROR);
      }
    ...
    Tcl_AppendResult(interp, "possible lbboundary parameters: wall, sphere, "
                             "cylinder, rhomboid, pore, stomatocyte, "
                             "hollow_cone, voxel, delete {c} to delete "
                             "lbboundary",
                     (char *)NULL);
    return (TCL_ERROR);
  }
  ...
  Tcl_AppendResult(interp, "LB_BOUNDARIES/LB_BOUNDARIES_GPU not compiled in!",
                   (char *)NULL);
  return (TCL_ERROR);
#endif /* LB_BOUNDARIES or LB_BOUNDARIES_GPU */
}
